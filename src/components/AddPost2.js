...
function AddPost2() {
    ...
    useEffect(()=>{
        if(url){
            ...
            .then(data=>{
                M.toast({html:"Post2 created successfully!",classes:"#43a047 green darken-1"})
                console.log(data)
                history.push("/")
...
return (
    <div>
         <div className="neu card post-card" >
                <h2 style={{color: "#ed6663"}}>Create a Departement</h2>
                <input required type="text" onChange={(e)=>setTitle(e.target.value)} placeholder="Title" maxLength = "30"/>
                <input required type="text" onChange={(e)=>setTag(e.target.value)} placeholder="Budget" maxLength = "90"/><br></br>
                ...
                <input id="filesFor" style={{visibility:"visible"}} required type="file" onChange={(e)=>setImage(e.target.files[0])} placeholder="upload  image: "></input><br></br>
                <input required contentEditable = "true" className="btn-neu" maxLength="4000" onChange={(e)=>setBody(e.target.value)} placeholder=" Description 
                "></input>
